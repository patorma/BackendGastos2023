...
public class NotaController {
	...
	public ResponseEntity<?> update(@Valid @RequestBody Nota nota,BindingResult result,@PathVariable Long id) throws Exception{
		//se obtiene los datos de la nota que se quiere modificar
		
		Nota notaActual = notaService.listarPorId(id);
		
		//nota con los datos actualizado
		...
		try {
			//modificamos los datos de la nota actual con los datos de la nota que te envian
			notaActual.setDescription(nota.getDescription());
			notaActual.setFecha(nota.getFecha());
	...
	public ResponseEntity<?> delete(@PathVariable Long id) throws Exception{
		...
		try {
			...
			response.put("mensaje", "Error al eliminar la nota en la base de datos!");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response,HttpStatus.INTERNAL_SERVER_ERROR);
		}
